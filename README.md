...
## 7. 차세대 다관점 하네스 엔지니어링 (Active Research)

Apple의 폐쇄형 GEMM 커널과 정면 승부하는 대신, **시스템과 알고리즘 상위 레벨에서 연산 자체를 제거하는 3대 전략**을 하네스 기반 실측으로 점진적 검증 중입니다:

1. **TeaCache / DeepCache (스텝 간 블록 스킵)**:
   - 디퓨전 후기 스텝에서 텐서 변화량이 5% 미만인 DiT 블록을 자동 감지하여 연산 스킵 (`H3_BLOCK_PROBE=1` 기반 프로파일링 완료, **2~2.5배 가속 목표**).
2. **매크로 청크 SDPA (복사 70초 완전 제거)**:
   - 프레임 단위 슬라이싱 대신 18프레임 매크로 청크로 배치화하여, Apple AME 하드웨어 가속을 100% 누리면서 VRAM 복사 오버헤드 완전 0초화.
3. **비대칭 레이어 스케줄링 (Asymmetric Stepping)**:
   - Step 0~1(50층 전층, 오디오/구도 확정) → Step 2~3(25층 감축, 영상 디테일만 정제)으로 음질 손실 없이 **1.4배 추가 가속**.
