...
1. **CPU 그래프 인코딩이 새 병목입니다** (160~187초). GPU를 79배 줄이자 그동안 가려져 있던 CPU 준비 작업이 드러났습니다. `MPSGraphExecutable` 로 한 번만 컴파일하면 줄어들 여지가 큽니다.
2. **화질 기준을 아직 통과하지 못했습니다.** SSIM 0.717~0.734 는 비결정성 바닥(0.82~0.87)보다 낮습니다 — 실행 간 편차를 넘어서는 **실제 차이**가 있다는 뜻입니다. 떨림은 오히려 줄었지만, 미리 정해둔 채택 기준(SSIM ≥ 0.87)에는 미달입니다. 더 넓은 반경을 측정 중입니다.

> 판정 기준을 **측정 전에** 적어두는 이유가 여기 있습니다. 속도가 79배로 나오면 화질 기준을 느슨하게 해석하고 싶어집니다. 기준을 먼저 고정해두면 그럴 수 없습니다.
