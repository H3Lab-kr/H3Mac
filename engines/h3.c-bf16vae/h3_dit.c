...
/* ── 희소 윈도우 어텐션 (H3_SPARSE_WIN=반경) ─────────────────────────
 * P0 프로브 실측: 모든 헤드가 어텐션 질량의 98.6%(평균)를 ±4 잠재 프레임과
 * 싱크(텍스트·오디오·조건 행)에 둔다. far(±8 초과) 질량은 0.000 이었다.
 * 그래서 비디오 쿼리는 [싱크 전체 + 이웃 프레임 윈도우] 만 보고,
 * 싱크 쿼리(오디오 포함 — 성역)는 전체를 밀집으로 본다.
 * 청크 C=반경 W: 청크 경계 쿼리도 최소 ±W 프레임을 보장한다.
 * 토큰이 시간 우선 배열이라 윈도우 키가 연속 → 복사 두 번이면 K/V 가 완성된다. */
static int sparse_window_radius(void) {
    const char *v = getenv("H3_SPARSE_WIN");
    if (!v || !*v) return 0;
    int r = atoi(v);
    return r >= 1 && r <= 16 ? r : 0;
}
...
static uint32_t sparse_spatial(const h3_dit *dit) {
    return ((uint32_t)dit->latent_h / 2u) * ((uint32_t)dit->latent_w / 2u);
}

static int sparse_attention_ready(h3_dit *dit, uint32_t rows, int radius) {
    static uint32_t reported_rows = 0;
    const char *why = NULL;
    ...
    /* 윈도우가 전체를 덮으면 밀집과 같으므로 이득이 없다 */
    else if (frames <= (uint32_t)radius * 3u) why = "too-short";
    if (reported_rows != rows) {
        reported_rows = rows;
        fprintf(stderr,
            "h3: sparse window %s (start=%u grid=%u frames=%u rows=%u)\n",
            why ? why : "active", video_start, spatial, frames, rows);
    }
    return why == NULL;
}
...
/* ── P0 어텐션 프로브 ──────────────────────────────────────────────
 * 희소 어텐션 타당성 조사용 1회성 계측 (H3_ATTN_PROBE=1 일 때만).
 * QKV 직후 커맨드 버퍼를 끊고 Q·K 를 읽어, 샘플 쿼리들의 어텐션 질량이
 * 어디로 가는지 헤드별로 잰다:
 *   frame = 같은 잠재 프레임(공간 헤드 후보)  ·  col = 같은 공간 위치(시간 헤드 후보)
 *   sink  = 비디오 타깃 이전 행(텍스트·오디오·조건)  ·  other = 나머지
 * 반드시 H3_DISABLE_INT8_QKV=1 H3_NAX=0 으로 돌린다 — 모튼 스위즐 레이아웃을 피해
 * Q/K 가 [rows, heads, dim] 행우선이 되게 하기 위해서다. */
static void attn_probe(h3_dit *dit, unsigned block, uint32_t rows,
                       char *error, size_t error_size) {
    enum { SAMPLES = 48 };
    if (dit->token_reduction_active) return;   /* 축약 좌표계는 다루지 않는다 */
    uint32_t video_start = dit->video_target_start;
    uint32_t video_rows = rows - video_start;
    uint32_t spatial = sparse_spatial(dit);
    if (!spatial || video_rows < spatial * 2) return;
    if (!h3_gpu_submit(dit->gpu)) {
        fprintf(stderr, "h3 attnprobe: submit failed: %s\n", h3_gpu_error(dit->gpu));
        return;
    }
    size_t count = (size_t)rows * INNER;
    uint16_t *q16 = malloc(count * sizeof(*q16));
    uint16_t *k16 = malloc(count * sizeof(*k16));
    float *kf = malloc(count * sizeof(*kf));
    if (q16 && k16 && kf &&
        h3_gpu_tensor_read_bf16(dit->query, q16, count) &&
        h3_gpu_tensor_read_bf16(dit->key, k16, count)) {
        for (size_t i = 0; i < count; i++) {
            uint32_t bits = (uint32_t)k16[i] << 16;
            memcpy(&kf[i], &bits, 4);
        }
        double scale = 1.0 / sqrt((double)HEAD_DIM);
        float *score = malloc((size_t)rows * sizeof(*score));
        double acc[HEADS][7];
        memset(acc, 0, sizeof(acc));
        for (unsigned s_i = 0; s_i < SAMPLES && score; s_i++) {
            uint32_t q_row = video_start +
                (uint32_t)(((uint64_t)(s_i + 1) * video_rows) / (SAMPLES + 1));
            uint32_t q_t = (q_row - video_start) / spatial;
            uint32_t q_xy = (q_row - video_start) % spatial;
            for (unsigned head = 0; head < HEADS; head++) {
                const uint16_t *q = q16 +
                    ((size_t)q_row * HEADS + head) * HEAD_DIM;
                float qf[HEAD_DIM];
                for (unsigned d = 0; d < HEAD_DIM; d++) {
                    uint32_t bits = (uint32_t)q[d] << 16;
                    memcpy(&qf[d], &bits, 4);
                }
                float peak = -1e30f;
                for (uint32_t r = 0; r < rows; r++) {
                    const float *k = kf +
                        ((size_t)r * HEADS + head) * HEAD_DIM;
                    float dot = 0.0f;
                    for (unsigned d = 0; d < HEAD_DIM; d++)
                        dot += qf[d] * k[d];
                    score[r] = dot * (float)scale;
                    if (score[r] > peak) peak = score[r];
                }
                /* 프레임 거리 히스토그램: dt=0,1,2,3~4,5~8,그 너머,싱크 */
                double total = 0.0, mass[7] = {0, 0, 0, 0, 0, 0, 0};
                for (uint32_t r = 0; r < rows; r++) {
                    double w = exp((double)score[r] - peak);
                    total += w;
                    if (r < video_start) { mass[6] += w; continue; }   /* sink */
                    uint32_t t = (r - video_start) / spatial;
                    uint32_t dt = t > q_t ? t - q_t : q_t - t;
                    int bucket = dt == 0 ? 0 : dt <= 2 ? 1 : dt <= 4 ? 2 :
                                 dt <= 8 ? 3 : dt <= 16 ? 4 : 5;
                    mass[bucket] += w;
                    (void)q_xy;
                }
                for (int c = 0; c < 7; c++)
                    acc[head][c] += total > 0.0 ? mass[c] / total : 0.0;
            }
        }
        for (unsigned head = 0; head < HEADS; head++)
            fprintf(stderr,
                "h3 attnprobe: block=%u head=%u d0=%.4f d12=%.4f d34=%.4f "
                "d58=%.4f d916=%.4f far=%.4f sink=%.4f\n", block, head,
                acc[head][0] / SAMPLES, acc[head][1] / SAMPLES,
                acc[head][2] / SAMPLES, acc[head][3] / SAMPLES,
                acc[head][4] / SAMPLES, acc[head][5] / SAMPLES,
                acc[head][6] / SAMPLES);
        free(score);
    } else {
        fprintf(stderr, "h3 attnprobe: read failed\n");
    }
    free(q16); free(k16); free(kf);
    if (!h3_gpu_begin(dit->gpu))
        fail(error, error_size, "attnprobe cannot reopen command buffer: %s",
             h3_gpu_error(dit->gpu));
}
...
#define OP(call, label) do {                                                    \
    ...
    if (step == 1 && getenv("H3_ATTN_PROBE"))
    ...
    int head_major_attention_output = int8_attention_output &&
        ...
        !getenv("H3_DISABLE_HEAD_MAJOR_ATTENTION_OUTPUT");
    if (sparse_active) {
        if (!run_sparse_attention(dit, rows, sparse_radius, error, error_size))
            return 0;
