...
/* ── 교차 길이 SDPA (희소 윈도우 어텐션용) ─────────────────────────
 * Q 행수와 K/V 행수가 다른 어텐션. 행우선 [1, rows, heads, dim] 전용.
 * 윈도우 어텐션은 "쿼리 청크 × (싱크+이웃 프레임 키)" 이므로 이 형태가 필요하다. */
static H3SDPA *h3_gpu_sdpa_cross_graph(H3GPU *gpu, uint32_t q_rows,
    ...
    @autoreleasepool {
        NSString *cacheKey = [NSString stringWithFormat:
                              @"cross:%u:%u:%u:%u:%u:%.9g",
                              (unsigned)dataType, q_rows, kv_rows, heads,
                              head_dim, scale];
        H3SDPA *cached = gpu.sdpaCache[cacheKey];
        if (cached) return cached;
        MPSGraph *graph = [[MPSGraph alloc] init];
        NSArray<NSNumber *> *qShape = @[@1, @(q_rows), @(heads), @(head_dim)];
        NSArray<NSNumber *> *kvShape = @[@1, @(kv_rows), @(heads), @(head_dim)];
        ...
        result.value = v;
        result.output = out;
        result.inputShape = qShape;      /* Q 형상 */
        result.outputShape = qShape;     /* 출력 = Q 형상 */
        gpu.sdpaCache[cacheKey] = result;
        return result;
    }
}
...
int h3_gpu_sdpa_cross_bf16(h3_gpu *opaque, h3_gpu_tensor *output,
    ...
    H3GPU *gpu = GPU(opaque);
    gpu.headMajorSDPAInputs = NO;
    size_t q_count = (size_t)q_rows * heads * head_dim;
    size_t kv_count = (size_t)kv_rows * heads * head_dim;
    if (!q_rows || !kv_rows || !heads || !head_dim ||
        !h3_gpu_require_command(gpu) ||
        !h3_gpu_require_bf16(gpu, query, q_count, @"cross SDPA query") ||
        !h3_gpu_require_bf16(gpu, key, kv_count, @"cross SDPA key") ||
        !h3_gpu_require_bf16(gpu, value, kv_count, @"cross SDPA value") ||
        !h3_gpu_require_bf16(gpu, output, q_count, @"cross SDPA output"))
        return 0;
    H3SDPA *cache = h3_gpu_sdpa_cross_graph(gpu, q_rows, kv_rows, heads,
                                            head_dim, scale,
                                            MPSDataTypeBFloat16);
    if (!cache) return 0;
    @autoreleasepool {
        ...
        @try {
            ...
            h3_gpu_set_error(gpu, @"MPSGraph cross SDPA failed: %@",
                             exception.reason);
            return 0;
        }
        gpu.command = command.rootCommandBuffer;
...
/* bf16 VAE 이식용 전방 선언 — 정의는 아래쪽에 static 으로 있다. */
static int h3_gpu_require_bf16(H3GPU *gpu, const h3_gpu_tensor *tensor,
                               size_t elements, NSString *label);

/* 비디오 VAE bf16 이식: 저장만 bf16, scale 은 채널 벡터라 f32 로 유지한다. */
int h3_gpu_scale_add_bf16(h3_gpu *opaque, h3_gpu_tensor *output,
    ...
    H3GPU *gpu = GPU(opaque);
    size_t count = (size_t)rows * width;
    if (!h3_gpu_require_bf16(gpu, residual, count, @"scale-add residual") ||
        !h3_gpu_require_bf16(gpu, branch, count, @"scale-add branch") ||
        !h3_gpu_require_elements(gpu, scale, width, @"scale-add scale") ||
        TENSOR(scale).dtype != H3_GPU_F32 ||
        !h3_gpu_require_bf16(gpu, output, count, @"scale-add output")) return 0;
    swiglu_args args = {rows, width};
...
/* 비디오 VAE bf16 이식. RoPE 회전과 RMS 통계는 셰이더 안에서 float 로 누산한다.
 * rope_cos/sin 은 f32 로 둔다 — 삼각 테이블이라 bf16 으로 내리면 회전각이 틀어진다. */
int h3_gpu_video_qkv_rope_bf16(h3_gpu *opaque, h3_gpu_tensor *query,
    ...
    size_t count = (size_t)sequence * inner;
    size_t rope_count = (size_t)sequence * rope_half;
    if (!h3_gpu_require_bf16(gpu, qkv, count * 3, @"video QKV") ||
        ...
        !h3_gpu_require_bf16(gpu, query, count, @"video query") ||
        !h3_gpu_require_bf16(gpu, key, count, @"video key") ||
        !h3_gpu_require_bf16(gpu, value, count, @"video value") ||
        rope_half * 2 > head_dim) return 0;
    qkv_args args = {sequence, heads, head_dim, rope_half, 0, epsilon};
