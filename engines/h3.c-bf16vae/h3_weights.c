...
/* 디스크에는 F32 로 저장돼 있지만 GPU 에서는 bf16 으로 쓰고 싶을 때.
 * 비디오 VAE 가중치가 그렇다. 변환은 로드 시점에 한 번뿐이라 런타임 비용이 없다. */
h3_gpu_tensor *h3_weight_load_f32_as_bf16(const h3_weight_store *store,
                                          h3_gpu *gpu, const char *name,
                                          int ndim, const uint64_t *shape,
                                          char *error, size_t error_size) {
    h3_gpu_tensor *source = load_tensor(store, gpu, name, ndim, shape,
                                        H3_DTYPE_F32, error, error_size);
    if (!source) return NULL;
    uint64_t elements = 1;
    for (int d = 0; d < ndim; d++) elements *= shape[d];
    h3_gpu_tensor *result = h3_gpu_tensor_new_bf16(gpu, (size_t)elements);
    if (!result) {
        fail(error, error_size, "cannot allocate bf16 copy of %s", name);
        h3_gpu_tensor_free(source);
        return NULL;
    }
    int ok = h3_gpu_begin(gpu) &&
             h3_gpu_cast_f32_to_bf16(gpu, result, source, (uint32_t)elements) &&
             h3_gpu_submit(gpu);
    h3_gpu_tensor_free(source);
    if (!ok) {
        fail(error, error_size, "cannot convert %s to bf16: %s", name,
             h3_gpu_error(gpu));
