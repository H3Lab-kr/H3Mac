...
#define FA_SG    2       /* threadgroup 당 simdgroup */
#define FA_KT   32       /* 키 타일 */
...
kernel void h3_flash_window_bf16(
    ...
    for (uint seg = 0; seg < 2; seg++) {
        ...
        for (uint kb = s0; kb < s1; kb += FA_KT) {
            ...
            for (uint i = tid; i < FA_KT * FA_D; i += FA_SG * 32) {
                uint r = i / FA_D, c = i % FA_D;
                bool ok = r < klen;
                kt[i] = ok ? Kh[(size_t)(kb + r) * FA_D + c] : bfloat(0);
                vt[i] = ok ? Vh[(size_t)(kb + r) * FA_D + c] : bfloat(0);
            }
            threadgroup_barrier(mem_flags::mem_threadgroup);
